...
/*
(1)내부 문자열 데이터에 대한 수정이 불가능하다.(즉, 내용 불변 Immutable)
문자열의 내용이 변경되면 새로운 객체를 생성하며
참조대상을 잃어버린 객체는 가비지(garbage) 컬렉션의 대상이 되고,
+ [garbage collection에 대한 심도 있는 이해는 
   https://d2.naver.com/helloworld/1329 를 참조해보자...]
새로운 객체에 새 주소를 할당하게 되므로 해시코드도 변하게 된다.

(2)String 클래스 간에 '+' 연산자를 사용하게 되면 
문자열 간의 결합이 가능하며, String 클래스와 다른 클래스 객체, 
또는 기본형 데이터 간에 '+' 연산자를 사용하는 경우에도
내부적으로 자동 String 클래스로 변환이 가능하다.

(3)String 은 내용을 변경할 수 없기 때문에 
...
(4)정적인 문자열을 처리하는 경우에는 주로 String 클래스를 사용,
동적인 문자열을 처리하는 경우에는
...
public class day24 {
	...
	public static void main(String[] args)
	{
		...
		//true
		//true
		//--주소값이 아니라 값의 내용을 비교한다.
		//  결론은 모든 결과값은 같다고 나올것이다.
