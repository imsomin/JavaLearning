...
-중첩 클래스를 감싸는 외부 클래스 {} 안에 static 을 붙인 새로운 클래스를 설계하는 것으로 모든 접근제어지시자를 사용할 수 있다.
-static 중첩 클래스가 포함하고 있는 메소드에서는 외부 클래스의 인스턴스 변수나 인스턴스 메소드에서는 접근할 수 없고
(외부 클래스의 객체를 생성하지 않은 상태) 클래스 변수와 클래스 메소드만 접근할 수 있다.
-프로그램의 구조를 보다 더 간단하고 알아보기 쉽게 구성할 수 있다.
-즉, static 으로 선언된 내부 클래스이다.
...
class Test
{
	...
	//static이 붙어있는 내부의 클래스
	public static class StaticNested  
	{
		int c = 30;

		void write()  //write가 메모리 할당 받았다면 c도 받았을것이다, 그러므로 c에 접근가능하다.
		{
...
public class day21 {
	...
	public static void main(String[] args)
	{
		...
		*/
		...
		//static 변수  
		//클래스를 읽어들일 때, 
		//클래스가 로딩이 되는 시점에서 메모리를 할당받는다.
		//클래스의 이름을 가지고 접근하게 된다.
		//그래서 Test.a; 만 해도
		//바로 접근 가능하다.
