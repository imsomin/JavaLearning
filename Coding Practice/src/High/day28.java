...
/*
-자바의 자료 구조 기능을 제공하는 클래스들 중 
가장 기본적인 클래스로 배열과 비슷하지만, 
배열과 달리 다른 종류의 데이터 형을 가진
데이터 요소를 가질 수 있으며, 
데이터가 가득차면 자동으로 
저장 영역을 확장시키게 된다.
(동질의 요소가 아니어도 괜찮고, 길이 변화 가능)
(배열과 완전히 다른 특징이다.)
(배열과는 그림의 형태가 비슷함을 의미)
(벡터는 오로지 객체만 저장할 수 있다)

-또한, 객체 레퍼런스를 저장하기 때문에
JDK1.5 이전에는 자바 기본형 데이터는 저장할 수 없었으나
...
- void insert(ElementAt(E obj, int index)
  주어진 위치에 객체를 저장한다.
...
public class day28 {
	...
	public static void main(String[] args)
	{
		...
		//인덱스 마지막(가장 끝)의 위치에 추가되는 것을 확인 가능

		
		//전체 요소 출력
		//'Iterator(이터레이터 or 아이터레이터)'
		//1.망보는 메소드()      -->  hasNext()
		//  다음 데이터 요소가 있는지의 여부를 반환
		//2.값을가져오는 메소드()  -->  next()
		//  다음 데이터 요소를 직접적으로 반환
